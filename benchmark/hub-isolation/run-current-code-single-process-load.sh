...
result_dir="benchmark/hub-isolation/results/current-code-single-process-load-$(date +%Y%m%d-%H%M%S)"
...
HUB_ROUTE_PLANNING_SCHEDULER_ENABLED=true \
HUB_ROUTE_BUILD_SCHEDULER_ENABLED=true \
HUB_ROUTE_BUILD_BATCH_SIZE="${HUB_ROUTE_BUILD_BATCH_SIZE:-2}" \
...
(
  while true; do
    docker stats --no-stream --format '{{.Name}} {{.CPUPerc}} {{.MemUsage}}' hub-rps-api hub-rps-postgres hub-rps-redis
    sleep 1
  done
...
new_hub_id="$(docker exec hub-rps-postgres psql -U testuser -d testdb -At -c "SELECT hub_id FROM p_hub WHERE name = '${new_hub_name}';")"
...
if [[ "${route_state}" != "200|0|0|0" ]]; then
  echo "single process route build did not complete: routes=${route_state}" >&2
  exit 1
fi
